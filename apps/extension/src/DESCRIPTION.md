...
| `api.ts`          | HTTP client for the web app's API surface (route handlers + tRPC). Adds the Bearer session token from `chrome.storage.local`. `normalizeApiBase` strips trailing slashes so popup + SW fetch URLs never double-slash.                                                                                  |
