...
| `api.ts`          | HTTP client for the web app's API surface (route handlers + tRPC). Adds the Bearer session token from `chrome.storage.local`. `normalizeApiBase` strips trailing slashes so popup + SW fetch URLs never double-slash. Calls use `redirect: 'manual'` and raise `AccessBlockedError` when an access proxy intercepts them (AIAGE-66). |
| `icon-hint.ts`    | Writes `tt:icon-hint` (running-timer count) so `background.js` can update the toolbar icon even when `/api/v1/timer` throws (`poll:error`, AIAGE-63).                                                                                                                                                                                |
| `storage.ts`      | Thin wrapper over `chrome.storage.local` for reads/writes of session, queue, and cached state.                                                                                                                                                                                                                                       |
| `sync.ts`         | Connects to `apps/ws`, applies incoming events to local state, wires reconnect via `packages/shared/src/ws/client.ts`. Owns the offline verdict: `isNetworkError` / `classifyReplayFailure` decide what may be queued, and only a genuine transport failure qualifies (AIAGE-66).                                                    |
