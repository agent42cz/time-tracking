...
- Started/finished: 2026-05-03
- pnpm workspaces, TypeScript strict, ESLint flat config (custom no-only-tests + no-console-in-src rules), Prettier, Husky pre-commit + lint-staged. `docker-compose.dev.yml` (Postgres 16 + Redis 7 + MailHog), `.github/workflows/ci.yml`, `scripts/test-trace.ts`. Layout per BUILD-PROMPT §5.
...
- Pending: full page wiring (timer, timesheet, dashboard, reports, members, audit, trash) — data layer is fully ready, the page shells consume the existing services. Playwright E2E is deferred to a follow-up iteration.
...
- Pending: full Clockify-style popup layout (PRD §10.3) — the queue + storage layer is done; layout per PRD §14.8 is verified manually with a real Chromium build.
...
- `ACCEPTANCE.md` maps every PRD §16 checkbox to the test file that proves it.
...
- **Live smoke against a real Coolify deploy.** The compose file validates and the Dockerfiles are well-formed, but the round-trip on a real Coolify host needs human verification (per PRD §14.8).
