...
- The web `AutoStackPreviewDialog` adds a "Ručně" tab with a start-time input so the user can pin the candidate's start time; the manual planner cascades the earlier blocker backward preserving its duration (US-82, US-84). Manual mode is constrained to the candidate's `endedAt` calendar day, so a manual start before midnight for a session stopped after midnight is rejected (`invalid_window`) by design.
