...
US-77..US-86 extend auto-stack to the Chrome extension and add a manual-mode tab to the web dialog. Key additions:

- `GET /api/v1/me` now includes `autoStackOverlaps: boolean` (US-77); the extension stores it and uses it to decide whether to check for overlaps after a stop.
- `POST /api/v1/timer/[id]/stop` returns `overlap: OverlapInfo | null` (US-78..US-80); `null` when the setting is OFF or there is no overlap.
